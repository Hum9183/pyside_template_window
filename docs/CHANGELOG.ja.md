...
### Changed
- インスタンスの GC 保護を `__init__` 内で自動的に行うように変更
  - `super().__init__()` の直後に `_instance` へ保存（UI 初期化中のエラーから保護）
  - GC 破棄条件を `__init__` の docstring に詳細に文書化

### Removed
- `set_instance()` クラスメソッドを削除（破壊的変更：内部実装として不要）
